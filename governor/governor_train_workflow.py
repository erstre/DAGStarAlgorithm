...
iot_devices_op = ["source"]
non_iot_op = ["tableRead", "annotation", "decisionTreeTrain", "multiVarLinearRegTrain", "blobWrite", "mqttPublish", "sink"]
...
json_file_path = "../network_sample/train_dataflow/" + str(num_nodes) + "_1/links_" + str(num_nodes) + "_1.json"
...
_, tmp = compute_shortest_path_dijkstra(COM_GRAPH, operator_mapping['source'], operator_mapping['tableRead'])
total_latency += tmp
total_latency += get_value_from_excel(file_path, 'tableRead', operator_mapping['tableRead'])

tmp1 = total_latency
tmp2 = total_latency

_, tmp = compute_shortest_path_dijkstra(COM_GRAPH, operator_mapping['tableRead'], operator_mapping['multiVarLinearRegTrain'])
tmp1 += tmp
tmp1 += get_value_from_excel(file_path, 'multiVarLinearRegTrain', operator_mapping['multiVarLinearRegTrain'])

_, tmp = compute_shortest_path_dijkstra(COM_GRAPH, operator_mapping['tableRead'], operator_mapping['annotation'])
tmp2 += tmp
tmp2 += get_value_from_excel(file_path, 'annotation', operator_mapping['annotation'])
_, tmp = compute_shortest_path_dijkstra(COM_GRAPH, operator_mapping['annotation'], operator_mapping['decisionTreeTrain'])
tmp2 += tmp
tmp2 += get_value_from_excel(file_path, 'decisionTreeTrain', operator_mapping['decisionTreeTrain'])


total_latency = tmp1 + tmp2

_, tmp = compute_shortest_path_dijkstra(COM_GRAPH, operator_mapping['multiVarLinearRegTrain'], operator_mapping['blobWrite'])
total_latency += tmp
_, tmp = compute_shortest_path_dijkstra(COM_GRAPH, operator_mapping['decisionTreeTrain'], operator_mapping['blobWrite'])
total_latency += tmp
total_latency += get_value_from_excel(file_path, 'blobWrite', operator_mapping['blobWrite'])
_, tmp = compute_shortest_path_dijkstra(COM_GRAPH, operator_mapping['blobWrite'], operator_mapping['mqttPublish'])
total_latency += tmp
