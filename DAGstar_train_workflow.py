...
def find_min_cost_of_conf(node_name, graph, c1, c2, c3, c4, c5, c6, c7 ,c8):
	...
	elif graph.nodes[node_name].get_operator() == 'blobWrite':
	...
	elif graph.nodes[node_name].get_operator() == 'annotation':
		return float(min_conf_of_code(c6))
	elif graph.nodes[node_name].get_operator() == 'tableRead':
...
def get_real_cost_of_conf(op_code, i, j, c1, c2, c3, c4, c5, c6, c7, c8):
	if op_code == "sink":
		return c1[i][j]
	elif op_code == "mqttPublish":
		return c2[i][j]
	elif op_code == "blobWrite":
		return c3[i][j]
	elif op_code == "decisionTree":
		return c4[i][j]
	elif op_code == "multiVarLinearReg":
		return c5[i][j]
	elif op_code == "annotation":
		return c6[i][j]
	elif op_code == "tableRead":
		return c7[i][j]
	elif op_code == "source":
		return c8[i][j]
	return -1
...
def calculate_heuristic(node_name, end_node, start_node, graph, c1, c2, c3, c4, c5, c6, c7, c8):
	...
	if node_name == end_node or node_name == start_node:
		...
		for my_parents in my_node.get_parents():
			...
			val2 = find_min_cost_of_conf(my_parents, graph, c1, c2, c3, c4, c5, c6, c7, c8)
			if max_heuristic_of_my_parents <= val1 + val2:
				max_heuristic_of_my_parents = val1 + val2
		return max_heuristic_of_my_parents

		# 	if max_heuristic_of_my_parents <= graph.nodes[my_parents].heuristic:
		# 		max_heuristic_of_my_parents = graph.nodes[my_parents].heuristic
		
		# #TODO: change the fixed_tables_per_code with a function that utilizes BO or RL or any other technology in order to get the predictions
		# my_heuristic = find_min_cost_of_conf(node_name, graph, c1, c2, c3, c4, c5, c6, c7, c8) + max_heuristic_of_my_parents
		# return my_heuristic

# Function for computing the heuristics in the right order
def bfs(graph, end_node, start_node, c1, c2, c3, c4, c5, c6, c7, c8):
	print("\n=================================")
	...
	while queue:
		...
		if node_name not in visited:
			...
			node.heuristic = calculate_heuristic(node_name, end_node, start_node, graph, c1, c2, c3, c4, c5, c6, c7, c8)
...
def construct_new_plan(so_far_nodes, new_node, original_graph, current_graph, possible_locations, possible_frameworks, END_NODE, c1, c2, c3, c4, c5, c6, c7, c8):
			...
			for j in range(0, len(possible_frameworks)):
				...
				for k in range(0, len(all_nodes_for_new_plan)):
					...
					# If this is the new_node then add the configuration based on the i and j values (type of location and framework accordingly) in the double loop
						...
						tmp_graph.add_node(new_node, get_real_cost_of_conf(original_graph.nodes[new_node].get_operator(), i, j, c1, c2, c3, c4, c5, c6, c7, c8), original_graph.nodes[new_node].get_heuristic_cost(), original_graph.nodes[new_node].get_operator(), possible_locations[i], possible_frameworks[j])
				# Once we have inserted all the nodes to the new graph, we need to insert the appropriate edges
				for k in range(0, len(all_nodes_for_new_plan)):
					# Get its neighbors (parents) that exist in the ORIGINAL graph
					for neighbor in original_graph.get_parents(all_nodes_for_new_plan[k]):
...
json_file_path = "network_sample/train_dataflow/" + str(my_conf_number) + "_1/network_" + str(my_conf_number) + "_1.json"  # Provide the path to your JSON file
...
json_file_path = "network_sample/train_dataflow/" + str(my_conf_number) + "_1/links_" + str(my_conf_number) + "_1.json"  # Provide the path to your JSON file
...
graph.add_node(3, 0, 0, 'blobWrite', 'dummy0', 'dummy0')
graph.add_node(4, 0, 0, 'decisionTree', 'dummy0', 'dummy0')
graph.add_node(5, 0, 0, 'multiVarLinearReg', 'dummy0', 'dummy0')
graph.add_node(6, 0, 0, 'annotation', 'dummy0', 'dummy0')
graph.add_node(7, 0, 0, 'tableRead', 'dummy0', 'dummy0')
graph.add_node(8, 0, 0, 'source', 'dummy0', 'dummy0')
...
bfs(graph, END_NODE, START_NODE, c1_table, c2_table, c3_table, c4_table, c5_table, c6_table, c7_table, c8_table)
		...
		for i in range(0, len(parent_nodes_plan_tmp)):
			...
			for j in range(0, len(parent_nodes_plan_tmp[i])):
				...
				if flag:
					...
					tmp_queue_of_plans = construct_new_plan(nodes_plan_tmp, parent_nodes_plan_tmp[i][j], graph_guru, plan_tmp, possible_locations, possible_frameworks, END_NODE, c1_table, c2_table, c3_table, c4_table, c5_table, c6_table, c7_table, c8_table)
