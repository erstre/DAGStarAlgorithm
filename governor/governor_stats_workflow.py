...
iot_devices_op = ["acc", "source"]
non_iot_op = ["average", "blobUpload", "slidingLinearReg", "senMLParse", "distinctCount", "zip", "plt", "kalmanFilter", "sink"]
...
json_file_path = "../network_sample/stats_dataflow/" + str(num_nodes) + "_1/links_" + str(num_nodes) + "_1.json"
...
tmp1 += tmp
tmp1 += get_value_from_excel(file_path, 'average', operator_mapping['average'])
_, tmp = compute_shortest_path_dijkstra(COM_GRAPH, operator_mapping['average'], operator_mapping['acc'])
tmp1 += tmp

_, tmp = compute_shortest_path_dijkstra(COM_GRAPH, operator_mapping['senMLParse'], operator_mapping['kalmanFilter'])
tmp2 += tmp
tmp2 += get_value_from_excel(file_path, 'kalmanFilter', operator_mapping['kalmanFilter'])
_, tmp = compute_shortest_path_dijkstra(COM_GRAPH, operator_mapping['kalmanFilter'], operator_mapping['slidingLinearReg'])
tmp2 += tmp
tmp2 += get_value_from_excel(file_path, 'slidingLinearReg', operator_mapping['slidingLinearReg'])
_, tmp = compute_shortest_path_dijkstra(COM_GRAPH, operator_mapping['slidingLinearReg'], operator_mapping['acc'])
tmp2 += tmp

_, tmp = compute_shortest_path_dijkstra(COM_GRAPH, operator_mapping['senMLParse'], operator_mapping['distinctCount'])
tmp3 += tmp
tmp3 += get_value_from_excel(file_path, 'distinctCount', operator_mapping['distinctCount'])
_, tmp = compute_shortest_path_dijkstra(COM_GRAPH, operator_mapping['distinctCount'], operator_mapping['acc'])
tmp3 += tmp


total_latency = max(tmp1, tmp2, tmp3)

total_latency += get_value_from_excel(file_path, 'acc', operator_mapping['acc'])
_, tmp = compute_shortest_path_dijkstra(COM_GRAPH, operator_mapping['acc'], operator_mapping['plt'])
total_latency += tmp
total_latency += get_value_from_excel(file_path, 'plt', operator_mapping['plt'])
_, tmp = compute_shortest_path_dijkstra(COM_GRAPH, operator_mapping['plt'], operator_mapping['zip'])
total_latency += tmp
total_latency += get_value_from_excel(file_path, 'zip', operator_mapping['zip'])
_, tmp = compute_shortest_path_dijkstra(COM_GRAPH, operator_mapping['zip'], operator_mapping['blobUpload'])
total_latency += tmp
total_latency += get_value_from_excel(file_path, 'blobUpload', operator_mapping['blobUpload'])
_, tmp = compute_shortest_path_dijkstra(COM_GRAPH, operator_mapping['blobUpload'], operator_mapping['sink'])
total_latency += tmp
