...
## Description
Across various domains, IoT applications depend on real-time analytics workflows to timely support decision-making procedures. The efficient execution of analytics workflows requires the utilization of the processing power available across the cloud to edge continuum. Nonetheless, suggesting the optimal workflow execution over a large network of heterogeneous devices is a challenging task. The increased IoT network size increases the complexity of the optimization problem at hand. The ingested data streams exhibit highly volatile properties. The population of network devices dynamically changes. We introduce DAG*, an A*-alike algorithm that prunes large amounts of the search space explored for suggesting the most efficient workflow execution with formal optimality guarantees. We also provide an incremental version of DAG*, a migration cost-aware DAG*, an end-to-end latency constrained DAG* and a parallel version of DAG*. All the aforementioned approaches retain the optimality property.

## Datasets
The ```datasets``` directory contains the utilized datasets. In detail, the train, etl, stats and pred datasets were generated from simulations run using the [iFogSim](https://onlinelibrary.wiley.com/doi/abs/10.1002/spe.2509) framework, while the Yahoo files were generated by deploying the Yahoo streaming benchmark workflow on real networks provided by [FIT IoT-LAB](https://www.iot-lab.info/). These datasets include detailed metrics, outputs, and performance evaluations.
...
The Experimental scenarios that were studied:
...
- Star topology: Necessary files can be found in the ```star_topology_scenario``` directory.
- NES-like architecture: Necessary files can be found in the ```NES_like_scenario``` directory.
- Migration-aware approach: Necessary files can be found in the ```migration-aware-dag-star``` directory.
- End-to-end latency constraint: Necessary files can be found in the ```latency-constrained-dag-star``` directory.
- Parallel approach: Necessary files can be found in the ```parallel-dag-star``` directory.
  > Note:
  > The parallel version of DAG* has been implemented in Java.
