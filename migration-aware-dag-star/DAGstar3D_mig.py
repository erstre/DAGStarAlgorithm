...
def create_com_graph(node1_lst, node2_lst, com_lat_list):
    ...
    print("\nEdges in communication graph (with latency):")
    for u, v, data in G.edges(data=True):
        print(f"{u} -- {v}, latency = {data['weight']}")

    return G

# Based on the CODE, i.e., operator this function returns the minimum cost
# across all (framework, location) configurations
def find_min_cost_of_conf(node_name, graph, operator_costs):
    operator_name = graph.nodes[node_name].get_operator()

    # END node has zero cost
    ...
    # Iterate over frameworks and locations
    for framework_costs in operator_costs[operator_name].values():
        for cost in framework_costs.values():
            if cost < min_cost:
                min_cost = cost

    if min_cost == float('inf'):
        raise ValueError(f"No valid cost found for operator: {operator_name}")

    return min_cost

# Function that returns the real cost based on the CODE of the operator and a specific configuration
def get_real_cost_of_conf(op_code, i, j, operator_costs, possible_locations, possible_frameworks):
    # Validate operator
    if op_code not in operator_costs:
        oooooooooooooo
        return -1

    # Validate indices
    if i >= len(possible_locations) or j >= len(possible_frameworks):
        zzzzzzzzzzzzzz
        return -1

    location = possible_locations[i]
    framework = possible_frameworks[j]

    # Fetch cost if it exists
    return operator_costs[op_code].get(framework, {}).get(location, -1)
...
def find_migration(tmp_graph, previous_opt_graph):
    """
    Finds the maximum communication cost between locations of common nodes
    in two graphs.
    """

    # Find common node names
    common_nodes = set(tmp_graph.nodes.keys()).intersection(
        set(previous_opt_graph.nodes.keys())
    )

    max_cost = 0

    for node_name in common_nodes:
        node_tmp = tmp_graph.nodes[node_name]
        node_prev = previous_opt_graph.nodes[node_name]

        loc_tmp = node_tmp.get_location()
        loc_prev = node_prev.get_location()

        # If locations are the same, cost is zero
        if loc_tmp == loc_prev:
            cost = 0
        else:
            cost, _ = get_path_cost(COM_GRAPH, loc_tmp, loc_prev)
...
def construct_new_plan(queue_of_plans, counter_heapQ, init_search, previous_opt_graph, so_far_nodes, new_node, original_graph, current_graph, possible_locations, possible_frameworks, END_NODE, operator_costs, aggr_func, com_lat_mult):
	...
	if new_node == END_NODE:
		...
		tmp_cost = compute_estimate_cost_of_plan(tmp_graph)
		if init_search == False:
			mig_cost = find_migration(tmp_graph, previous_opt_graph)
			tmp_graph.set_estimated_cost_of_graph(tmp_cost + mig_cost)
		else:
			tmp_graph.set_estimated_cost_of_graph(tmp_cost)

		if tmp_graph.get_estimated_cost_of_graph() <= LATENCY_THRESHOLD:
			counter_heapQ += 1
			heapq.heappush(queue_of_plans, (tmp_graph.estimated_cost, counter_heapQ, tmp_graph))
			tmp_graph.set_counterQ(counter_heapQ)
	...
	# If this is not the END_NODE
		...
		for i in range(0, len(possible_locations)):
			for j in range(0, len(possible_frameworks)):
				#If statement that checks pairs of flink
				if possible_frameworks[j] == 'flink' and possible_locations[i][0:2] == 'a8':
					continue 
				...
				for k in range(0, len(all_nodes_for_new_plan)):
					...
					# If this is the new_node then add the configuration based on the i and j values (type of location and framework accordingly) in the double loop
						...
						tmp_graph.add_node(new_node, get_real_cost_of_conf(original_graph.nodes[new_node].get_operator(), i, j, operator_costs, possible_locations, possible_frameworks), original_graph.nodes[new_node].get_heuristic_cost(), original_graph.nodes[new_node].get_operator(), possible_locations[i], possible_frameworks[j])
				# Once we have inserted all the nodes to the new graph, we need to insert the appropriate edges
				for k in range(0, len(all_nodes_for_new_plan)):
					# Get its neighbors (parents) that exist in the ORIGINAL graph
					for neighbor in original_graph.get_parents(all_nodes_for_new_plan[k]):
	...
	return queue_of_plans, counter_heapQ
...
def update_priority_queue(old_queue, nodes_to_change, guru, END_NODE, START_NODE, operator_costs, old_min, new_min, previous_opt_graph):
	...
	for tmp_var in old_queue:
		...
		mig_cost = find_migration(plan, previous_opt_graph)
		plan.set_estimated_cost_of_graph(tmp_cost + mig_cost)
...
json_file_path = "fitiot/" + str(my_conf_number) + "/network_" + str(my_conf_number) + "_1.json"  # Provide the path to your JSON file
...
json_file_path = "fitiot/" + str(my_conf_number) + "/links_" + str(my_conf_number) + "_1.json"  # Provide the path to your JSON file
...
for operator in data['operators']:
    ...
    codes_dictionary[operator_name] = {
        ...
        'possible_frameworks_lst': frameworks
    }
...
##########################################
# Read framework-dependent Excel files
##########################################

framework_excel_files = {
    "python": f"yahoo_dataset/yahoo_xlsx/python/{word}_{my_conf_number}_python.xlsx",
    "flink":  f"yahoo_dataset/yahoo_xlsx/flink/{word}_{my_conf_number}_flink.xlsx",
    "java":   f"yahoo_dataset/yahoo_xlsx/java/{word}_{my_conf_number}_java.xlsx",
}
...
for operator in data['operators']:
    operator_costs[operator['name']] = {}

##########################################
# Load costs
##########################################

for framework, excel_file in framework_excel_files.items():
    df = pd.read_excel(excel_file)

    for _, row in df.iterrows():
        operator_name = row.iloc[0]
        costs = row.iloc[1:].tolist()

        if operator_name not in operator_costs:
            continue

        operator_info = next(
            (op for op in data['operators'] if op['name'] == operator_name),
            None
        )

        operator_costs[operator_name][framework] = {}

        for site, cost in zip(site_names, costs):
            if cost == -1:
                continue

            cost *= mult_factor

            # Fixed operator → only first site
            if operator_info and operator_info['fixed']:
                if site == site_names[0]:
                    operator_costs[operator_name][framework][site] = cost
            else:
                operator_costs[operator_name][framework][site] = cost
...
print("\nOperator costs (operator → framework → site → cost):\n")
for op, fw_data in operator_costs.items():
    print(op)
    for fw, site_data in fw_data.items():
        print(f"  {fw}: {site_data}")
...
# Define the init configuration
previous_opt_graph = copy.deepcopy(graph)
print(previous_opt_graph.nodes.keys())

# sink
previous_opt_graph.nodes[1].set_location("a8-43.grenoble")
previous_opt_graph.nodes[1].set_framework("java")

# aggregation
previous_opt_graph.nodes[2].set_location("a8-134.grenoble")
previous_opt_graph.nodes[2].set_framework("java")

#join
previous_opt_graph.nodes[3].set_location("a8-212.grenoble")
previous_opt_graph.nodes[3].set_framework("python")

# map
previous_opt_graph.nodes[4].set_location("a8-156.grenoble")
previous_opt_graph.nodes[4].set_framework("java")

# redis-source
previous_opt_graph.nodes[5].set_location("a8-165.grenoble")
previous_opt_graph.nodes[5].set_framework("java")

# filter
previous_opt_graph.nodes[6].set_location("a8-153.grenoble")
previous_opt_graph.nodes[6].set_framework("python")

# source
previous_opt_graph.nodes[7].set_location("a8-152.grenoble")
previous_opt_graph.nodes[7].set_framework("python")

print()
print("My INIT Graph Structure (or Workflow):")
print(previous_opt_graph)
...
while True:
	...
	while len(queue_of_plans) > 0:
		q_size.append(len(queue_of_plans))
		...
		for i in range(0, len(parent_nodes_plan_tmp)):
			...
			for j in range(0, len(parent_nodes_plan_tmp[i])):
				...
				if flag:
					...
					queue_of_plans, counter_heapQ = construct_new_plan(queue_of_plans, counter_heapQ, init_search, previous_opt_graph, nodes_plan_tmp, parent_nodes_plan_tmp[i][j], graph_guru, plan_tmp, possible_locations, possible_frameworks, END_NODE, operator_costs, aggr_func, com_lat_mult)
		...
		# queue_of_plans = queue_of_plans[:4000]
		# print(f"The size of queue currently is -----> {len(queue_of_plans)} ...")
		##############################################################

	if len(queue_of_plans) == 0:
		print()
		print("No solution found...")
		print()
		print("Mean size of priority queue:")
		print(statistics.mean(q_size))
		print("Median size of priority queue:")
		print(statistics.median(q_size))
	...
	print(opt_plan)
	print()
	print("The cost of this plan (with migration) is:")
	print(opt_plan.get_estimated_cost_of_graph())
	print()
	# print(q_size)
	print()
	print("Mean size of priority queue:")
	...
	#############################################################
	# For experiment with placements one after the other
	#############################################################
