...
def find_min_cost_of_conf(node_name, graph, c1, c2, c3, c4, c5, c6, c7 ,c8, c9, c10, c11):
	if graph.nodes[node_name].get_operator() == 'END':
		return 0
	if graph.nodes[node_name].get_operator() == 'sink':
		return float(min_conf_of_code(c1))
	elif graph.nodes[node_name].get_operator() == 'blobUpload':
		return float(min_conf_of_code(c2))
	elif graph.nodes[node_name].get_operator() == 'zip':
		return float(min_conf_of_code(c3))
	elif graph.nodes[node_name].get_operator() == 'slidingLinearReg':
		return float(min_conf_of_code(c4))
	elif graph.nodes[node_name].get_operator() == 'kalmanFilter':
	...
	elif graph.nodes[node_name].get_operator() == 'distinctCount':
		return float(min_conf_of_code(c9))
	elif graph.nodes[node_name].get_operator() == 'plt':
		return float(min_conf_of_code(c10))
	elif graph.nodes[node_name].get_operator() == 'acc':
		return float(min_conf_of_code(c11))
...
def get_real_cost_of_conf(op_code, i, j, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11):
	if op_code == "sink":
		return c1[i][j]
	elif op_code == "blobUpload":
		return c2[i][j]
	elif op_code == "zip":
		return c3[i][j]
	elif op_code == "slidingLinearReg":
		return c4[i][j]
	elif op_code == "kalmanFilter":
		return c5[i][j]
...
def calculate_heuristic(node_name, end_node, start_node, graph, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11):
	...
	if node_name == end_node or node_name == start_node:
		...
		for my_parents in my_node.get_parents():
			...
			val2 = find_min_cost_of_conf(my_parents, graph, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11)
...
def bfs(graph, end_node, start_node, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11):
	print("\n=================================")
	...
	while queue:
		...
		if node_name not in visited:
			...
			node.heuristic = calculate_heuristic(node_name, end_node, start_node, graph, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11)
...
def construct_new_plan(so_far_nodes, new_node, original_graph, current_graph, possible_locations, possible_frameworks, END_NODE, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11):
			...
			for j in range(0, len(possible_frameworks)):
				...
				for k in range(0, len(all_nodes_for_new_plan)):
					...
					# If this is the new_node then add the configuration based on the i and j values (type of location and framework accordingly) in the double loop
						...
						tmp_graph.add_node(new_node, get_real_cost_of_conf(original_graph.nodes[new_node].get_operator(), i, j, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11), original_graph.nodes[new_node].get_heuristic_cost(), original_graph.nodes[new_node].get_operator(), possible_locations[i], possible_frameworks[j])
				# Once we have inserted all the nodes to the new graph, we need to insert the appropriate edges
				for k in range(0, len(all_nodes_for_new_plan)):
					# Get its neighbors (parents) that exist in the ORIGINAL graph
					for neighbor in original_graph.get_parents(all_nodes_for_new_plan[k]):
...
def update_priority_queue(old_queue, nodes_to_change, guru, END_NODE, START_NODE, c1_table, c2_table, c3_table, c4_table, c5_table, c6_table, c7_table, c8_table, c9_table, c10_table, c11_table, old_min, new_min):
	...
	if new_min != old_min:
		...
		bfs(guru_upd, END_NODE, START_NODE, c1_table, c2_table, c3_table, c4_table, c5_table, c6_table, c7_table, c8_table, c9_table, c10_table, c11_table)
...
json_file_path = "network_sample/stats_dataflow/" + str(my_conf_number) + "_1/network_" + str(my_conf_number) + "_1.json"  # Provide the path to your JSON file
...
json_file_path = "network_sample/stats_dataflow/" + str(my_conf_number) + "_1/links_" + str(my_conf_number) + "_1.json"  # Provide the path to your JSON file
...
code1_costs = read_row("datasets/stats_" + str(my_conf_number) + "_dataflow.xlsx", 7)
rows  = my_conf_number
cols = 1
c1_table = []
# Iterate over rows
...
code2_costs = read_row("datasets/stats_" + str(my_conf_number) + "_dataflow.xlsx", 6)
rows = 1
cols = 1
c2_table = []
for i in range(rows):
	row = []
	for j in range(cols):
		row.append(code2_costs[0] * mult_factor)
	c2_table.append(row)

code3_costs = read_row("datasets/stats_" + str(my_conf_number) + "_dataflow.xlsx", 2)
rows = 1
cols = 1
c3_table = []
for i in range(rows):
...
code4_costs = read_row("datasets/stats_" + str(my_conf_number) + "_dataflow.xlsx", 10)
rows = 1
cols = 1
c4_table = []
for i in range(rows):
...
code5_costs = read_row("datasets/stats_" + str(my_conf_number) + "_dataflow.xlsx", 8)
rows  = my_conf_number
cols = 1
c5_table = []
# Iterate over rows
for i in range(rows):
    ...
    for j in range(cols):
        # Append the corresponding random integer to the row
        row.append(code5_costs[i] * mult_factor)
    # Append the row to the table
    c5_table.append(row)

code6_costs = read_row("datasets/stats_" + str(my_conf_number) + "_dataflow.xlsx", 12)
rows = 1
cols = 1
c6_table = []
for i in range(rows):
	row = []
	for j in range(cols):
		row.append(code6_costs[0] * mult_factor)
	c6_table.append(row)

code7_costs = read_row("datasets/stats_" + str(my_conf_number) + "_dataflow.xlsx", 11)
rows  = my_conf_number
cols = 1
c7_table = []
# Iterate over rows
...
code8_costs = read_row("datasets/stats_" + str(my_conf_number) + "_dataflow.xlsx", 4)
rows  = my_conf_number
cols = 1
c8_table = []
# Iterate over rows
...
code9_costs = read_row("datasets/stats_" + str(my_conf_number) + "_dataflow.xlsx", 5)
rows  = my_conf_number
cols = 1
c9_table = []
# Iterate over rows
for i in range(rows):
    ...
    for j in range(cols):
        # Append the corresponding random integer to the row
        row.append(code9_costs[i] * mult_factor)
    # Append the row to the table
    c9_table.append(row)

code10_costs = read_row("datasets/stats_" + str(my_conf_number) + "_dataflow.xlsx", 9)
rows = 1
cols = 1
c10_table = []
for i in range(rows):
...
code11_costs = read_row("datasets/stats_" + str(my_conf_number) + "_dataflow.xlsx", 3)
rows  = my_conf_number
cols = 1
c11_table = []
# Iterate over rows
for i in range(rows):
    ...
    for j in range(cols):
        # Append the corresponding random integer to the row
        row.append(code11_costs[i] * mult_factor)
    # Append the row to the table
    c11_table.append(row)
...
graph.add_node(2, 0, 0, 'blobUpload', 'dummy0', 'dummy0')
graph.add_node(3, 0, 0, 'zip', 'dummy0', 'dummy0')
graph.add_node(4, 0, 0, 'slidingLinearReg', 'dummy0', 'dummy0')
graph.add_node(5, 0, 0, 'kalmanFilter', 'dummy0', 'dummy0')
graph.add_node(6, 0, 0, 'senMLParse', 'dummy0', 'dummy0')
graph.add_node(7, 0, 0, 'source', 'dummy0', 'dummy0')
graph.add_node(8, 0, 0, 'average', 'dummy0', 'dummy0')
graph.add_node(9, 0, 0, 'distinctCount', 'dummy0', 'dummy0')
graph.add_node(10, 0, 0, 'plt', 'dummy0', 'dummy0')
graph.add_node(11, 0, 0, 'acc', 'dummy0', 'dummy0')

##########################################
# Extrection, Transform & Load dataflow  (STATS)
##########################################
...
graph.add_edge(11, 4, 1)
graph.add_edge(11, 8, 1)
...
bfs(graph, END_NODE, START_NODE, c1_table, c2_table, c3_table, c4_table, c5_table, c6_table, c7_table, c8_table, c9_table, c10_table, c11_table)
...
while True:
		...
		for i in range(0, len(parent_nodes_plan_tmp)):
			...
			for j in range(0, len(parent_nodes_plan_tmp[i])):
				...
				if flag:
					...
					tmp_queue_of_plans = construct_new_plan(nodes_plan_tmp, parent_nodes_plan_tmp[i][j], graph_guru, plan_tmp, possible_locations, possible_frameworks, END_NODE, c1_table, c2_table, c3_table, c4_table, c5_table, c6_table, c7_table, c8_table, c9_table, c10_table, c11_table)
	...
	queue_of_plans.append(opt_plan)
	...
	change_lst = [0.01, 0.1, 10, 10, 10, 0.1, 10]
	...
	# 	if graph.nodes[node].get_operator() == 'distinctCount':
	# 		nodes_to_change.add(node)
	# print(nodes_to_change)
	# old_min = min_conf_of_code(c9_table)
	# for i in range(0, len(c9_table)):
	# 	c9_table[i][0] = c9_table[i][0] * change_lst[counter_of_changes]
	# new_min = min_conf_of_code(c9_table)
	##############################################################
	##############################################################
	
	if counter_of_changes == 0 or counter_of_changes == 2 or counter_of_changes == 6:
		nodes_to_change = set()

		for node in graph.get_all_nodes():
			if graph.nodes[node].get_operator() == 'average':
				nodes_to_change.add(node)
		
		print(nodes_to_change)


		# Change 1: Add a new location that changes the heuristic
		# The modeled change is the addition of a new option for the location of blobUpload operators
		old_min = min_conf_of_code(c8_table)
		for i in range(0, len(c8_table)):
			c8_table[i][0] = c8_table[i][0] * change_lst[counter_of_changes]
		new_min = min_conf_of_code(c8_table)
	elif counter_of_changes == 1 or counter_of_changes == 4:
		nodes_to_change = set()
		...
		for node in graph.get_all_nodes():
			if graph.nodes[node].get_operator() == 'kalmanFilter':
				nodes_to_change.add(node)
		...
		# Change 1: Add a new location that changes the heuristic
		# The modeled change is the addition of a new option for the location of blobUpload operators
		old_min = min_conf_of_code(c5_table)
		for i in range(0, len(c5_table)):
			c5_table[i][0] = c5_table[i][0] * change_lst[counter_of_changes]
		new_min = min_conf_of_code(c5_table)


	counter_of_changes = counter_of_changes + 1

	new_priority_queue, graph_guru = update_priority_queue(queue_of_plans, nodes_to_change, graph_guru, END_NODE, START_NODE, c1_table, c2_table, c3_table, c4_table, c5_table, c6_table, c7_table, c8_table, c9_table, c10_table, c11_table, old_min, new_min)
	
	queue_of_plans.clear()
	
	queue_of_plans.extend(new_priority_queue)
