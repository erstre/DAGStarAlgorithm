...
def find_min_cost_of_conf(node_name, graph, c1, c2, c3, c4, c5, c6, c7 ,c8, c9, c10, c11):
	...
	elif graph.nodes[node_name].get_operator() == 'azureTableInsert':
		return float(min_conf_of_code(c3))
	elif graph.nodes[node_name].get_operator() == 'csvToSenml':
		return float(min_conf_of_code(c4))
	elif graph.nodes[node_name].get_operator() == 'annotate':
		return float(min_conf_of_code(c5))
	elif graph.nodes[node_name].get_operator() == 'join':
		return float(min_conf_of_code(c6))
	elif graph.nodes[node_name].get_operator() == 'interpolation':
		return float(min_conf_of_code(c7))
	elif graph.nodes[node_name].get_operator() == 'bloomFilter':
		return float(min_conf_of_code(c8))
	elif graph.nodes[node_name].get_operator() == 'rangeFilter':
...
def get_real_cost_of_conf(op_code, i, j, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11):
	if op_code == "sink":
		...
		return c2[i][j]
	elif op_code == "azureTableInsert":
		return c3[i][j]
	elif op_code == "csvToSenml":
		return c4[i][j]
	elif op_code == "annotate":
		return c5[i][j]
	elif op_code == "join":
		return c6[i][j]
	elif op_code == "interpolation":
		return c7[i][j]
	elif op_code == "bloomFilter":
		return c8[i][j]
	elif op_code == "rangeFilter":
		return c9[i][j]
...
def update_priority_queue(old_queue, nodes_to_change, guru, END_NODE, START_NODE, c1_table, c2_table, c3_table, c4_table, c5_table, c6_table, old_min, new_min):
	...
	if new_min != old_min:
		...
		bfs(guru_upd, END_NODE, START_NODE, c1_table, c2_table, c3_table, c4_table, c5_table, c6_table, c7_table, c8_table)
...
json_file_path = "network_sample/etl_dataflow/" + str(my_conf_number) + "_1/network_" + str(my_conf_number) + "_1.json"  # Provide the path to your JSON file
...
json_file_path = "network_sample/etl_dataflow/" + str(my_conf_number) + "_1/links_" + str(my_conf_number) + "_1.json"  # Provide the path to your JSON file
...
graph.add_node(3, 0, 0, 'azureTableInsert', 'dummy0', 'dummy0')
graph.add_node(4, 0, 0, 'csvToSenml', 'dummy0', 'dummy0')
graph.add_node(5, 0, 0, 'annotate', 'dummy0', 'dummy0')
graph.add_node(6, 0, 0, 'join', 'dummy0', 'dummy0')
graph.add_node(7, 0, 0, 'interpolation', 'dummy0', 'dummy0')
graph.add_node(8, 0, 0, 'bloomFilter', 'dummy0', 'dummy0')
graph.add_node(9, 0, 0, 'rangeFilter', 'dummy0', 'dummy0')
graph.add_node(10, 0, 0, 'senmlParse', 'dummy0', 'dummy0')
graph.add_node(11, 0, 0, 'source', 'dummy0', 'dummy0')

##########################################
# Extrection, Transform & Load dataflow  (ETL)
##########################################
...
while True:
	...
	for node in graph.get_all_nodes():
		if graph.nodes[node].get_operator() == 'mqttPublish':
	...
	# The modeled change is the addition of a new option for the location of mqttPublish operators
	old_min = min_conf_of_code(c2_table)
	codes_dictionary['mqttPublish']['possible_locations_lst'].append('cluster3')
	c2_table.append([7, 9])
	new_min = min_conf_of_code(c2_table)

	# # Change 2: Add a new location that does not change the heuristic
	# # The modeled change is the addition of a new option for the location of mqttPublish operators
	# old_min = min_conf_of_code(c2_table)
	# codes_dictionary['CODE2']['possible_locations_lst'].append('cluster3')
	# c2_table.append([10, 19])
	# new_min = min_conf_of_code(c2_table)

	# # Change 3: Change-decrease the cost of a conf that changes the heuristic
	# # The modeled change is the modification of an existing option for the location of mqttPublish operators
	# old_min = min_conf_of_code(c2_table)
	# c2_table[0][1] = 2
	# new_min = min_conf_of_code(c2_table)

	# # Change 4: Change-decrease the cost of a conf that does not change the heuristic
	# # The modeled change is the modification of an existing option for the location of mqttPublish operators
	# old_min = min_conf_of_code(c2_table)
	# c2_table[1][1] = 12
	# new_min = min_conf_of_code(c2_table)

	# # Change 5: Remove a location that changes the heuristic
	# # The modeled change is the addition of a new option for the location of mqttPublish operators
	# old_min = min_conf_of_code(c2_table)
	# codes_dictionary['mqttPublish']['possible_locations_lst'].remove('cluster1')
	# del c2_table[0]
	# print(c2_table)
	# new_min = min_conf_of_code(c2_table)

	# # Change 6: Remove a location that does not change the heuristic
	# # The modeled change is the addition of a new option for the location of mqttPublish operators
	# old_min = min_conf_of_code(c2_table)
	# codes_dictionary['mqttPublish']['possible_locations_lst'].remove('cluster2')
	# del c2_table[1]
	# print(c2_table)
	# new_min = min_conf_of_code(c2_table)

	new_priority_queue, graph_guru = update_priority_queue(queue_of_plans, nodes_to_change, graph_guru, END_NODE, START_NODE, c1_table, c2_table, c3_table, c4_table, c5_table, c6_table, old_min, new_min)
